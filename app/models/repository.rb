  ...
  def sync_updateinfo
    ...
    updateinfo_xml.elements('update').each do |update_hash|
      ...
      update_hash[:pkglist][:collection][:package].each do |package_hash|
        ...
        # FIXME: Are new packages possible? Would they be in primary.xml of the update repo?
        next unless package

        # FIXME: handle
        # - kind <update from="maint-coord@suse.de" status="stable" type="recommended" version="1">
        # - status <update from="maint-coord@suse.de" status="stable" type="recommended" version="1">
        # - created_at: package_hash[:issued] <issued date="1683799293"/>
        # - issues: package_hash[:references]
        patch = package.patches.find_or_build_by(name: package_hash[:id])
        patch.assign_attributes(package_hash.slice(:title, :severity, :description))
        patch.save!
      end
    end
  end
